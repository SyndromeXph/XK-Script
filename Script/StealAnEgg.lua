local tx = os.clock()
local Kye = select(2, pcall(getconnections, game:GetService("LogService").MessageOut))
if type(Kye) ~= "table" then
	Kye = {}
end
for _, c in ipairs(Kye) do
	pcall(
        function()
		local f = c.Function
		...
		local key, msd = pcall(
                hookfunction, f, function(m, t)
			if type(m) == "string" and (m:lower():find("lur", 1, true) or m:find("internal", 1, true) or m:match(":%d+[:\n]")) then
				return
			end
			return msd and msd(m, t)
		end)
		if not key then
			c:Disable()
		end
...
print(string.format("[Bypass_L by_du8] Luar bypass %.2fs", os.clock() - tx))
...
task.spawn(function()
    local colorA = Color3.fromRGB(0, 0, 0)
...
local function createGradientBorder(window)
    ...
    gradientStroke.Color = Color3.new(1, 1, 1)
...
local function startBorderAnimation(window, speed)
    ...
    local animation = game:GetService("RunService").Heartbeat:Connect(function()
        if not gradientStroke or gradientStroke.Parent == nil then
...
local function reinitializeBorder()
    ...
    if gradientStroke and borderEnabled then
...
local function setupWindowReopenListener()
    local function checkWindow()
        local mainFrame = Window.UIElements.Main
        if mainFrame and not mainFrame:FindFirstChild("GradientStroke") and borderEnabled then
            reinitializeBorder()
        end
    end
...
setupWindowReopenListener()


local mainSec = Window:Section({ Title = "主要的", Opened = true })
local toolsSec = Window:Section({ Title = "杂项", Opened = true })

local InfoTab = mainSec:Tab({ Title = "信息", Icon = "info" })
InfoTab:Paragraph({
	Title = "Xk脚本 偷一个蛋",
	Desc = "1061153825(已满) 副群 1083698313",
})


local StealTab = mainSec:Tab({ Title = "自动偷蛋", Icon = "zap" })
local PlaceTab = mainSec:Tab({ Title = "放置与孵化", Icon = "package" })
local SelectTab = mainSec:Tab({ Title = "蛋选择", Icon = "list" })
local CharTab = toolsSec:Tab({ Title = "角色", Icon = "user" })
local SettingsTab = toolsSec:Tab({ Title = "设置", Icon = "settings" })

-- 自动偷蛋（功能与原 T4 / N4 / l4 / Q4 一致）
StealTab:Section({ Title = "模式" })
StealTab:Toggle({
	Title = "自动偷蛋（补间）",
	Desc = "飞过去偷蛋并存入背包。",
	Value = false,
...
-- 放置与孵化
PlaceTab:Section({ Title = "放置 / 孵化" })
PlaceTab:Button({
	Title = "放置蛋",
	Desc = "补间回家，放蛋并孵化。",
	Callback = function()
		task.spawn(function()
			h.statusText = "[手动] 正在放置蛋..."
			g4(h.glideSpeed)
			v4()
			u4()
			h.isReturning = false
...
-- 蛋选择（主要）— 多选列表，不是开关
SelectTab:Section({ Title = "刷蛋过滤器" })
SelectTab:Paragraph({
	Title = "使用方法",
	Desc = "在列表中选择区域和稀有度。若开启了「始终偷取 Secret+」，Secret+ 可跳过区域过滤。",
})
...
SelectTab:Toggle({
	Title = "始终偷取 Secret+",
	Desc = "即使未选择对应区域，也始终偷取 Secret/Eternal/Divine。",
